...
describe('6809 cpu', () => {
  ...
  describe('cpu operation', () => {
    ...
    each([
      [0x0, 'A', 0x20, [0x81, 0x20], cpus.ZERO, 2],
      [0x0, 'A', 0x20, [0x81, 0x21], cpus.NEGATIVE | cpus.CARRY, 2],
      [0x0, 'A', 0x20, [0x81, 0x1f], 0x00, 2],
      [0x0, 'B', 0x55, [0xc1, 0x55], cpus.ZERO, 2]],
    ).it(
    ...
    each([
      [0x0, 'A', 0x20, 0x10, [0x91, 0x20], cpus.ZERO, 0x1020, 4],
      [0x0, 'B', 0x55, 0x11, [0xd1, 0x55], cpus.ZERO, 0x1155, 4]],
    ).it(
    ...
    each([
      [0x0, 'A', 0x20, [0xb1, 0x10, 0x20], cpus.ZERO, 0x1020, 5],
      [0x0, 'B', 0x55, [0xf1, 0x11, 0xaa], cpus.ZERO, 0x11aa, 5]],
    ).it(
    ...
    each(
        [
          [0x0000, 'A', 0x20, [0xa1, 0x84], 'X', 0x1020, cpus.ZERO, 4],
          [0x0000, 'B', 0x55, [0xe1, 0x84], 'X', 0x1020, cpus.ZERO, 4],
        ],
    ...
    each([
      [0x0000, 'X', [0x8c, 0x55, 0xaa], 0x55aa, cpus.ZERO, 4],
      [0x0000, 'X', [0x8c, 0x55, 0xab], 0x55aa, cpus.NEGATIVE | cpus.CARRY, 4],
      [0x0000, 'X', [0x8c, 0x55, 0xaa], 0xaa55, cpus.OVERFLOW, 4],
      [0x0000, 'Y', [0x10, 0x8c, 0x55, 0xaa], 0x55aa, cpus.ZERO, 5],
      [0x0000, 'S', [0x11, 0x8c, 0x55, 0xaa], 0x55aa, cpus.ZERO, 5],
      [0x0000, 'U', [0x11, 0x83, 0x55, 0xaa], 0x55aa, cpus.ZERO, 5],
      [0x0000, 'D', [0x10, 0x83, 0x55, 0xaa], 0x55aa, cpus.ZERO, 5],
    ]).it(
    ...
    each([
      [0x0000, 'X', 0x10, [0x9c, 0xaa], 0x55aa, 0x10aa, cpus.ZERO, 6],
      [0x0000, 'Y', 0x11, [0x10, 0x9c, 0xaa], 0x55aa, 0x11aa, cpus.ZERO, 7],
      [0x0000, 'S', 0x12, [0x11, 0x9c, 0xaa], 0x55aa, 0x12aa, cpus.ZERO, 7],
      [0x0000, 'U', 0x13, [0x11, 0x93, 0xaa], 0x55aa, 0x13aa, cpus.ZERO, 7],
      [0x0000, 'D', 0x13, [0x10, 0x93, 0xaa], 0x55aa, 0x13aa, cpus.ZERO, 7],
    ]).it(
    ...
    each([
      [0x0000, 'X', [0xbc, 0x10, 0xaa], 0x55aa, 0x10aa, cpus.ZERO, 7],
      [0x0000, 'Y', [0x10, 0xbc, 0x11, 0xaa], 0x55aa, 0x11aa, cpus.ZERO, 8],
      [0x0000, 'S', [0x11, 0xbc, 0x12, 0xaa], 0x55aa, 0x12aa, cpus.ZERO, 8],
      [0x0000, 'U', [0x11, 0xb3, 0x13, 0xaa], 0x55aa, 0x13aa, cpus.ZERO, 8],
      [0x0000, 'D', [0x10, 0xb3, 0x13, 0xaa], 0x55aa, 0x13aa, cpus.ZERO, 8],
    ]).it(
        'Compares a 16 bit register against direct memory',
    ...
    each([
      [0x0000, 'X', 'Y', [0xac, 0xa4], 0x55aa, 0x10aa, cpus.ZERO, 6],
      [0x0000, 'Y', 'X', [0x10, 0xac, 0x84], 0x55aa, 0x11aa, cpus.ZERO, 7],
      [0x0000, 'S', 'X', [0x11, 0xac, 0x84], 0x55aa, 0x12aa, cpus.ZERO, 7],
      [0x0000, 'U', 'X', [0x11, 0xa3, 0x84], 0x55aa, 0x13aa, cpus.ZERO, 7],
      [0x0000, 'D', 'X', [0x10, 0xa3, 0x84], 0x55aa, 0x13aa, cpus.ZERO, 7],
    ]).it(
    ...
    each(
        [
          // [0x0000, 'A', [0xa0, 0x84, 0x55], 'X', 0x0002, 0xff, 0xaa, 4],
          // [0x0000, 'B', [0xe0, 0xa4, 0xaa], 'Y', 0x0002, 0xff, 0x55, 4],
          [0x0000, 'D', [0xa3, 0x84, 0x55, 0xaa], 'X', 0x0002, 0xffff, 0xaa55, 6],
        ],
    ).it('subtracts indexed value from the object register',
        (
    ...
    each(
        [
          [0x0000, [0x6d, 0x84, 0x00], 'X', 0x0002, cpus.ZERO, 6],
        ],
    ).it('tests an indexed memory address',
        (address, code, index, atAddress, ccFlags, cycles) => {
          loadMemory(address, code);
          subject.registers.get('PC').set(address);
          subject.registers.get(index).set(atAddress);
          const cycleCount = runToNext(subject);
    ...
    it('correctly branches when register<memory when compared', () => {
      const code = [0x8e, 0x04, 0xfc, 0xcc, 0x20, 0x20, 0xed, 0x81, 0x8c, 0x06, 0x00, 0x2d, 0xf9];
      loadMemory(0x2000, code);
      subject.PC.set(0x2000);
      runToNext(subject);
      runToNext(subject);
      runToNext(subject);
      runToNext(subject);
      runToNext(subject);
      expect(subject.PC.fetch()).toBe(0x2006);
      expect(subject.registers.get('X').fetch()).toBe(0x04fe);
      expect(subject.registers.get('D').fetch()).toBe(0x2020);
    });
  });
