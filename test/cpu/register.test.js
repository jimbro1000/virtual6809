...
describe('register prototype', () => {
  it(
      'requires name and size plus the control register at construction',
      () => {
        const sample = new CpuRegister(cpus.SHORT, 'A');
  ...
  describe('8 bit behaviour', () => {
    let sample;
    beforeEach(() => {
      sample = new CpuRegister(cpus.SHORT, 'A');
    });
  ...
  describe('16 bit behaviour', () => {
    let sample;
    beforeEach(() => {
      sample = new CpuRegister(cpus.LONG, 'X');
    });
  ...
  describe('cc interaction', () => {
    ...
    beforeEach(() => {
      ...
      sample = new CpuRegister(cpus.SHORT, 'A', cc);
    });
...
describe('d register', () => {
  it('combines the A and B registers', () => {
    const a = new CpuRegister(cpus.SHORT, 'A', undefined);
    const b = new CpuRegister(cpus.SHORT, 'B', undefined);
    a.load(0xaa);
    b.load(0x55);
    const d = new WideRegister('D', a, b);
