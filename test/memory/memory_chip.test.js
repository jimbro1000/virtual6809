...
describe('memory chip abstract', () => {
  it('requires a type and size at construction', () => {
    const subject = new Chip(chips.RAM, chips.K8);
  ...
  each([-1, 32768, 65535]).
      it('throws an error if the read address is out of range', (address) => {
        const subject = new Chip(chips.RAM, chips.K32);
        expect(() => {
  ...
  it('does not modify value at given address if the memory is not writeable',
      () => {
