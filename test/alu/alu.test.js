...
describe("Arithmetic Logic Unit", () => {
    ...
    describe("Adds two registers, ", () => {
        each([[0x01,0x02,0x03], [0xff,0x01,0x00]]).
        it("adds two 8 bit registers for an 8 bit result", (s1, s2, expected) => {
            const result = subject.add8(s1,s2);
            expect(result).toBe(expected);
        });
        each([[0x7f,0x02,cpus.NEGATIVE], [0xff,0x01,0]]).
        it("sets negative flag if the resulting 8 bit msb is set", (s1,s2,negative) => {
            subject.add8(s1,s2);
            expect(cc.save() & cpus.NEGATIVE).toBe(negative);
        });
        each([[0xff,0x01,cpus.CARRY], [0x7f,0x01,0]]).
        it("sets carry flag if the result of the 8 bit sum exceeds 8 bits", (s1,s2,carry) => {
            subject.add8(s1,s2);
            expect(cc.save() & cpus.CARRY).toBe(carry);
        });
        each([[0xff,0x01,cpus.ZERO], [0x01,0x00,0]]).
        it("sets zero flag if the result of the 8 bit sum is zero", (s1,s2,zero) => {
            subject.add8(s1,s2);
            expect(cc.save() & cpus.ZERO).toBe(zero);
        });
        each([[0xf8,0x08,cpus.HALFCARRY],[0x04,0x03,0]]).
        it("sets half-carry if the result of the 4 bit sum exceeds 4 bits", (s1,s2,half) => {
            subject.add8(s1,s2);
            expect(cc.save() & cpus.HALFCARRY).toBe(half);
        });
        each([[0x80,0x80,cpus.OVERFLOW],[0x40,0x40,cpus.OVERFLOW],
                   [0xff,0xfe,0]]).
        ...
        each([[0x0001,0x0002,0x0003], [0x00ff,0x0001,0x0100], [0xffff,0x0001,0x0000]]).
        it("adds two 16 bit registers for an 16 bit result", (s1, s2, expected) => {
            const result = subject.add16(s1,s2);
            expect(result).toBe(expected);
        });
        each([[0x7fff,0x0002,cpus.NEGATIVE], [0xffff,0x0001,0]]).
        it("sets negative flag if the resulting 16 bit msb is set", (s1,s2,negative) => {
            subject.add16(s1,s2);
            expect(cc.save() & cpus.NEGATIVE).toBe(negative);
        });
        each([[0xffff,0x01,cpus.CARRY], [0x7fff,0x01,0]]).
        it("sets carry flag if the result of the 8 bit sum exceeds 8 bits", (s1,s2,carry) => {
            subject.add16(s1,s2);
            expect(cc.save() & cpus.CARRY).toBe(carry);
        });
        each([[0xffff,0x0001,cpus.ZERO], [0x0001,0x0000,0]]).
        it("sets zero flag if the result of the 16 bit sum is zero", (s1,s2,zero) => {
            subject.add16(s1,s2);
            expect(cc.save() & cpus.ZERO).toBe(zero);
        });
        each([[0x00f8,0x0008,0,0],[0x00f8,0x0008,cpus.HALFCARRY,cpus.HALFCARRY]]).
        it("does not modify the half carry flag on a 16 bit sum", (s1,s2,state,half) => {
            cc.load(state);
            subject.add16(s1,s2);
            expect(cc.save() & cpus.HALFCARRY).toBe(half);
        });
        each([[0x8000,0x8000,cpus.OVERFLOW],[0x4000,0x4000,cpus.OVERFLOW],[0xffff,0xfffe,0]]).
        it("sets overflow if the sign of the result is not as expected", (s1,s2,overflow) => {
            subject.add16(s1,s2);
            expect(cc.save() & cpus.OVERFLOW).toBe(overflow);
        });
